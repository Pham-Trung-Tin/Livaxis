  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-neutral-50 px-4">
      <div className="w-full max-w-md rounded-xl border border-neutral-200 bg-white p-6 shadow-sm">
        <h1 className="mb-2 text-2xl font-semibold">{t('auth.resetFormTitle')}</h1>
        <p className="mb-6 text-sm text-neutral-500">{t('auth.resetFormDesc')}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? t('auth.resetBtnUpdating') : t('auth.resetBtnTitle')}
          </button>
        </form>
      </div>
    </div>
